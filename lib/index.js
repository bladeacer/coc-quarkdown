...
async function activate(context) {
  ...
  const setupDefaultKeymaps = async () => {
    ...
    for (const map of maps) {
      const hasMapping = await nvim.call("maparg", [map.key, "n"]);
      if (!hasMapping) {
  ...
  context.subscriptions.push(
    ...
    import_coc2.commands.registerCommand("coc-quarkdown.compile", async () => {
      ...
      if (doc) {
        ...
        const path = import_coc2.Uri.parse(doc.uri).fsPath.replace(/ /g, "\\ ");
  ...
  const currentDoc = await import_coc2.workspace.document;
